// src/lib/fetchers/fetchTopMenu.js

/**
 * Fetches top menu from BFF and returns only top-level items
 * with their children already embedded.
 *
 * WordPress path format: /category/india/national-trends/
 * We strip /category/ prefix → /india/national-trends/
 * Then strip trailing slash  → /india/national-trends
 */

function cleanPath(wpPath = "") {
  return wpPath.replace(/^\/category/, "").replace(/\/$/, "") || "/";
}

export async function fetchTopMenu() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/posts/topmenu/`,
      {
        next: { revalidate: 3600 }, // cache menu for 1 hour
      }
    );

    if (!res.ok) return [];

    const json = await res.json();
    console.log("menu items");
    console.log(json);
    const nodes = json?.data?.nodes ?? [];
    // Only keep top-level items (parentId === null)
    const topLevel = nodes.filter((n) => n.parentId === null);
    // Map to clean shape
    return topLevel.map((item) => ({
      key: item.key,
      label: item.label,
      path: cleanPath(item.path),
      children: (item.childItems?.nodes ?? []).map((child) => ({
        label: child.label,
        path: cleanPath(child.path),
      })),
    }));
  } catch (err) {
    console.error("[fetchTopMenu] failed:", err);
    return [];
  }
}